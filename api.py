...
class U2020Api(Telnet):
    def __init__(self, ftp_auth: tuple, nbi_auth: tuple, oss_name: str, oss_ip: str, ftp_port: int, nbi_port: int,
    ...
    def auto(self, nbi_connect_timeout=60, nbi_command_timeout=3600):
        self.upload_to_oss()
        self.execute_on_oss(connect_timeout=nbi_connect_timeout, command_timeout=nbi_command_timeout)
        self.download_from_oss()

    def upload_to_oss(self):
        path_to_scripts_on_oss = "/export/home/sysm/ftproot/itf_n/nms_mml_server/script/"
        try:
            with paramiko.Transport(self.oss_ip, self.ftp_port) as transport:
                ...
                try:
                    ...
                    logging.critical(f"Exception in upload_to_oss U2020Api in For Cycle, "
            ...
            with paramiko.Transport(self.oss_ip, self.ftp_port) as transport:
                ...
                try:
                    for result_file in list_dir:
                        if result_file.find(user_name) != -1 and result_file.find('.rstprocess') == -1:
                            sftp.get(f"{oss_result_path}{result_file}", f"{local_result_path}\\{result_file}")
                            logging.info(f'Success Download File {result_file}')
                    sftp.close()
                    transport.close()
                except Exception as error:
                    logging.critical(f"Exception in download_from_oss U2020Api in For Cycle, "
    ...
    def execute_on_oss(self, connect_timeout: int, command_timeout: int):
        ...
        try:
            ...
            if answer.find(success) != -1:
                ...
                logging.critical(f"Else in telnet_nbi U2020Api not Find Success by {self.oss_name}")
        ...
        try:
            for file_name in self.files_dict.keys():
                ...
                answer = self.read_until(finish.encode('ascii'), timeout=command_timeout)
                answer = answer.decode('ascii')
                answers += answer
                if answer.find(success) != -1:
        ...
        try:
            ...
            logging.critical(f"Exception in telnet_nbi U2020Api Can not Logout: {error}")
            logging.info(f'Answers: {answers}')
            self.close()
            return
